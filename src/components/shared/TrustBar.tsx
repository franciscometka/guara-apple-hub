import { ShieldCheck, FileCheck2, Wrench, MessageCircle, type LucideIcon } from "lucide-react";
...
export function TrustBar() {
  return (
    <div className="border-b border-border bg-background py-6 md:py-8">
      <Container>
        <RevealGroup className="grid grid-cols-2 gap-5 md:grid-cols-4 md:gap-8" stagger={0.06}>
              ...
              <RevealItem key={item}>
                <div className="group flex items-center gap-3 transition-transform duration-300 hover:-translate-y-0.5">
                  <Icon
                    ...
                    aria-hidden="true"
                  />
                  <span className="text-sm leading-tight text-muted-foreground">{item}</span>
                </div>
              </RevealItem>
            );
          })}
